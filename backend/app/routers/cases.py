...
def update_case(case_id: int, case_update: CaseUpdate, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    # Permission check
    if current_user.rol == UserRole.OPERADOR:
        if db_case.creado_por_id != current_user.id:
             raise HTTPException(status_code=403, detail="Operators can only edit their own cases")
        # Operators might be restricted in what they can update, but for simplicity let's allow basic updates
        # Or strictly follow: "OPERADOR: puede crear casos y editar sólo los que creó."
